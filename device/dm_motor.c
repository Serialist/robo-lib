...
float DJI_Motor_Encoder_To_Anglesum(DJI_Motor_Data_Typedef *, float, uint16_t);

float DJI_Motor_Encoder_To_Angle(DJI_Motor_Data_Typedef *, float, uint16_t);
 ...
 * @retval None
 */
void DJI_Motor_Info_Update(uint32_t *Identifier, uint8_t *buf, DJI_Motor_Info_Typedef *data)
{

	data.encode = ((int16_t)buf[0] << 8 | (int16_t)buf[1]);
	data.velocity = ((int16_t)buf[2] << 8 | (int16_t)buf[3]);
	data.current = ((int16_t)buf[4] << 8 | (int16_t)buf[5]);
