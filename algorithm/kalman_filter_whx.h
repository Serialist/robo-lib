...
typedef struct kf_t
{
	...
	MAT32 xhat;	   // x(k|k)
	MAT32 xhatminus; // x(k|k-1)
	MAT32 u;		   // control vector u
	MAT32 z;		   // measurement vector z
	MAT32 P;		   // covariance matrix P(k|k)
	MAT32 Pminus;	   // covariance matrix P(k|k-1)
	MAT32 F, FT;	   // state transition matrix F FT
	MAT32 B;		   // control matrix B
	MAT32 H, HT;	   // measurement matrix H
	MAT32 Q;		   // process noise covariance matrix Q
	MAT32 R;		   // measurement noise covariance matrix R
	MAT32 K;		   // kalman gain  K
	MAT32 S, temp_matrix, temp_matrix1, temp_vector, temp_vector1;
