...
void whxKalman_Filter_Init(whxKalmanFilter_t *kf, uint8_t xhatSize, uint8_t uSize, uint8_t zSize)
{
    ...
    MAT32_INIT(&kf->xhat, kf->xhatSize, 1, (float *)kf->xhat_data);

    // xhatminus x(k|k-1)
    kf->xhatminus_data = (float *)user_malloc(sizeof_float * xhatSize);
    memset(kf->xhatminus_data, 0, sizeof_float * xhatSize);
    MAT32_INIT(&kf->xhatminus, kf->xhatSize, 1, (float *)kf->xhatminus_data);
    ...
    MAT32_INIT(&kf->P, kf->xhatSize, kf->xhatSize, (float *)kf->P_data);
    ...
    MAT32_INIT(&kf->Pminus, kf->xhatSize, kf->xhatSize, (float *)kf->Pminus_data);
    ...
    MAT32_INIT(&kf->F, kf->xhatSize, kf->xhatSize, (float *)kf->F_data);
    MAT32_INIT(&kf->FT, kf->xhatSize, kf->xhatSize, (float *)kf->FT_data);
    ...
    if (uSize != 0)
    {
        ...
        MAT32_INIT(&kf->B, kf->xhatSize, kf->uSize, (float *)kf->B_data);
    }
    ...
    MAT32_INIT(&kf->S, kf->xhatSize, kf->xhatSize, (float *)kf->S_data);
    MAT32_INIT(&kf->temp_matrix, kf->xhatSize, kf->xhatSize, (float *)kf->temp_matrix_data);
    MAT32_INIT(&kf->temp_matrix1, kf->xhatSize, kf->xhatSize, (float *)kf->temp_matrix_data1);
    MAT32_INIT(&kf->temp_vector, kf->xhatSize, 1, (float *)kf->temp_vector_data);
    MAT32_INIT(&kf->temp_vector1, kf->xhatSize, 1, (float *)kf->temp_vector_data1);
...
void whxKalman_Filter_xhatMinusUpdate(whxKalmanFilter_t *kf)
{
    if (!kf->SkipEq1)
    {
        if (kf->uSize > 0)
        {
            ...
            kf->MatStatus = MAT32_MULT(&kf->F, &kf->xhat, &kf->temp_vector);
            kf->temp_vector1.numRows = kf->xhatSize;
            kf->temp_vector1.numCols = 1;
            kf->MatStatus = MAT32_MULT(&kf->B, &kf->u, &kf->temp_vector1);
            kf->MatStatus = MAT32_ADD(&kf->temp_vector, &kf->temp_vector1, &kf->xhatminus);
        }
        else
        {
            kf->MatStatus = MAT32_MULT(&kf->F, &kf->xhat, &kf->xhatminus);
        }
    }
}
...
void whxKalman_Filter_PminusUpdate(whxKalmanFilter_t *kf)
{
    if (!kf->SkipEq2)
    {
        kf->MatStatus = MAT32_TRANS(&kf->F, &kf->FT);
        kf->MatStatus = MAT32_MULT(&kf->F, &kf->P, &kf->Pminus);
        kf->temp_matrix.numRows = kf->Pminus.numRows;
        kf->temp_matrix.numCols = kf->FT.numCols;
        kf->MatStatus = MAT32_MULT(&kf->Pminus, &kf->FT, &kf->temp_matrix); // temp_matrix = F P(k-1) FT
...
void whxKalman_Filter_SetK(whxKalmanFilter_t *kf)
{
    if (!kf->SkipEq3)
    {
        kf->MatStatus = MAT32_TRANS(&kf->H, &kf->HT); // z|x => x|z
        kf->temp_matrix.numRows = kf->H.numRows;
        kf->temp_matrix.numCols = kf->Pminus.numCols;
        kf->MatStatus = MAT32_MULT(&kf->H, &kf->Pminus, &kf->temp_matrix); // temp_matrix = H·P'(k)
        kf->temp_matrix1.numRows = kf->temp_matrix.numRows;
        kf->temp_matrix1.numCols = kf->HT.numCols;
        kf->MatStatus = MAT32_MULT(&kf->temp_matrix, &kf->HT, &kf->temp_matrix1); // temp_matrix1 = H·P'(k)·HT
        kf->S.numRows = kf->R.numRows;
        kf->S.numCols = kf->R.numCols;
        kf->MatStatus = MAT32_ADD(&kf->temp_matrix1, &kf->R, &kf->S); // S = H P'(k) HT + R
        kf->MatStatus = MAT32_INVERSE(&kf->S, &kf->temp_matrix1);     // temp_matrix1 = inv(H·P'(k)·HT + R)
        kf->temp_matrix.numRows = kf->Pminus.numRows;
        kf->temp_matrix.numCols = kf->HT.numCols;
        kf->MatStatus = MAT32_MULT(&kf->Pminus, &kf->HT, &kf->temp_matrix); // temp_matrix = P'(k)·HT
...
void whxKalman_Filter_xhatUpdate(whxKalmanFilter_t *kf)
{
    if (!kf->SkipEq4)
    {
        ...
        kf->temp_vector.numCols = 1;
        kf->MatStatus = MAT32_MULT(&kf->H, &kf->xhatminus, &kf->temp_vector); // temp_vector = H xhat'(k)
        kf->temp_vector1.numRows = kf->z.numRows;
        kf->temp_vector1.numCols = 1;
        kf->MatStatus = MAT32_SUB(&kf->z, &kf->temp_vector, &kf->temp_vector1); // temp_vector1 = z(k) - H·xhat'(k)
        kf->temp_vector.numRows = kf->K.numRows;
        kf->temp_vector.numCols = 1;
        kf->MatStatus = MAT32_MULT(&kf->K, &kf->temp_vector1, &kf->temp_vector); // temp_vector = K(k)·(z(k) - H·xhat'(k))
...
void whxKalman_Filter_P_Update(whxKalmanFilter_t *kf)
{
    if (!kf->SkipEq5)
    {
        ...
        kf->MatStatus = MAT32_MULT(&kf->K, &kf->H, &kf->temp_matrix);                 // temp_matrix = K(k)·H
        kf->MatStatus = MAT32_MULT(&kf->temp_matrix, &kf->Pminus, &kf->temp_matrix1); // temp_matrix1 = K(k)·H·P'(k)
        kf->MatStatus = MAT32_SUB(&kf->Pminus, &kf->temp_matrix1, &kf->P);
    }
}
