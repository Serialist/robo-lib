...
static void QuaternionEKF_A_Update(KalmanFilter_Info_TypeDef *KF){
  ...
  /* Limit the P data */
  Clampf(KF->Data.P[28],-10000,10000);
  Clampf(KF->Data.P[35],-10000,10000);
...
static void QuaternionEKF_xhat_Update(KalmanFilter_Info_TypeDef *KF){
  ...
  {
    Clampf(KF->Data.Cache_Vector[0][4],-1e-2f*KF->dt,1e-2f*KF->dt);
    Clampf(KF->Data.Cache_Vector[0][5],-1e-2f*KF->dt,1e-2f*KF->dt);
  }
