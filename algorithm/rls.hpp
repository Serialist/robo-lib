...
public:
	...
	const Matrixf<dim, 1>& update(Matrixf<dim, 1>& sampleVector, float actualOutput) {
		...
		paramsVector += gainVector
			* (actualOutput - (sampleVector.trans() * paramsVector)[0][0]); // Get params vector
		transMatrix = (transMatrix - gainVector * sampleVector.trans() * transMatrix)
			/ lambda; // Get transferred matrix
