				 ...
				 float dt) // 计算theta和d_theta给lqr用，同时也计算腿长L0
{
	...
	vmc->lBD = sqrt((vmc->XD - vmc->XB) * (vmc->XD - vmc->XB)
					+ (vmc->YD - vmc->YB) * (vmc->YD - vmc->YB));
	...
	vmc->phi2 =
		...
		* atan2f(
			(vmc->B0
			 + sqrt(vmc->A0 * vmc->A0 + vmc->B0 * vmc->B0 - vmc->C0 * vmc->C0)),
			vmc->A0 + vmc->C0);
	vmc->phi3 = atan2f(vmc->YB - vmc->YD + vmc->l2 * SINF(vmc->phi2),
					   vmc->XB - vmc->XD + vmc->l2 * COSF(vmc->phi2));
	// C点直角坐标
	vmc->XC = vmc->l1 * COSF(vmc->phi1) + vmc->l2 * COSF(vmc->phi2);
	vmc->YC = vmc->l1 * SINF(vmc->phi1) + vmc->l2 * SINF(vmc->phi2);
	// C点极坐标
	vmc->L0 = sqrt((vmc->XC - vmc->l5 / 2.0f) * (vmc->XC - vmc->l5 / 2.0f)
				   + vmc->YC * vmc->YC);

	vmc->phi0 = atan2f(
		vmc->YC, (vmc->XC - vmc->l5 / 2.0f)); // phi0用于计算lqr需要的theta
	...
	vmc->d_phi0 = (vmc->phi0 - vmc->last_phi0)
				  / dt; // 计算phi0变化率，d_phi0用于计算lqr需要的d_theta
...
void VMC_5bar_IK(VMC_t *vmc, float tp, float f0)
{
	...
	vmc->j11 =
		(vmc->l1 * SINF(vmc->phi0 - vmc->phi3) * SINF(vmc->phi1 - vmc->phi2))
		/ SINF(vmc->phi3 - vmc->phi2);
	vmc->j12 =
		(vmc->l1 * COSF(vmc->phi0 - vmc->phi3) * SINF(vmc->phi1 - vmc->phi2))
		/ (vmc->L0 * SINF(vmc->phi3 - vmc->phi2));
	vmc->j21 =
		(vmc->l4 * SINF(vmc->phi0 - vmc->phi2) * SINF(vmc->phi3 - vmc->phi4))
		/ SINF(vmc->phi3 - vmc->phi2);
	vmc->j22 =
		(vmc->l4 * COSF(vmc->phi0 - vmc->phi2) * SINF(vmc->phi3 - vmc->phi4))
		/ (vmc->L0 * SINF(vmc->phi3 - vmc->phi2));

	vmc->torque_set[FRONT] =
		vmc->j11 * vmc->F0
		+ vmc->j12
			  * vmc->Tp; // 得到RightFront的输出轴期望力矩，F0为五连杆机构末端沿腿的推力
	vmc->torque_set[BACK] =
		vmc->j21 * vmc->F0
		+ vmc->j22
			  * vmc->Tp; // 得到RightBack的输出轴期望力矩，Tp为沿中心轴的力矩
}
...
{
	float fn =
		leg->F0 * COSF(leg->theta) + leg->Tp * SINF(leg->theta) / leg->L0
		+ -0.6f
			  * (az + (-leg->dd_L0 * COSF(leg->theta))
